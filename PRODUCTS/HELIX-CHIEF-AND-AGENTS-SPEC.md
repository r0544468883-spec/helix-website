...
### כלל הגבול (encapsulation — הכי חשוב) 🔒
**HELIX CHIEF לעולם לא נוגע באייג'נטים הפנימיים של מוצר.** לכל מוצר יש **צ'יף-מחלקתי אחד** — וזה ה-*single interface* היחיד ש-CHIEF מכיר. CHIEF מבקש תוצאה ("תכתוב מאמר על X"); הצ'יף-המחלקתי מחליט **לבד** איך (fast/team, מי חוקר, מי מבקר, כמה סבבים). המוצר הוא **קופסה שחורה** מבחינת CHIEF.

```
HELIX CHIEF  ──►  Department Chief (Rank)  ──►  [Researcher · Maker · Critic]
   (מאקרו)          (הגבול — הקו האמצעי)              (מיקרו — CHIEF לא רואה)
```

**למה:**
1. **אנקפסולציה** — Rank משנה את הצוות הפנימי שלו מ-6 ל-4 מומחים → CHIEF לא מרגיש, ה-contract לא משתנה.
2. **stand-alone בחינם** — הצ'יף-המחלקתי חייב להתקיים ממילא כשלקוח קנה רק את המוצר הבודד. ה"מוח" של המוצר שייך למוצר, לא מושאל מ-CHIEF.
3. **contract דק** — CHIEF↔מוצר מדבר רק ב-`input/actions/result/entitlement/autonomy` (§7 P0). כל השאר פנימי.
