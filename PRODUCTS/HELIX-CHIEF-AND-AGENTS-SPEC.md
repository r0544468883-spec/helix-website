...
> **הבחנה קריטית:** §4 מגדיר תזמור ברמת-**מאקרו** (CHIEF מנתב בין מוצרים). סעיף זה מגדיר את הרמת-**מיקרו**: מה קורה *בתוך* מוצר בודד. עד היום כל מוצר = agent יחיד עם system-prompt שמגלם כמה תפקידים. מהיום כל מוצר = **מחלקה של אייג'נטים מתמחים**, שכל אחד חושב עצמאית, תחת orchestrator פנימי. זו ארכיטקטורה **פרקטלית**: אותו דפוס "מנצח + צוות + מבקר" חוזר בשתי הרמות (CHIEF↔מחלקות, orchestrator↔תפקידים).
...
4. **Orchestrator/Chief-of-dept** — מתזמר את השלושה, מחליט fast/team, מחזיק את היעד, עושה merge סופי.
...
lib/agents/<product>/
  ...
  orchestrator.ts    // מחליט fast/team, מתזמר, merge
  roles/
