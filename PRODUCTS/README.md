...
## הנכס הסודי (הבידול האמיתי)
לא הטכנולוגיה — **ההפצה**. הקולגה כבר מדבר עם מאות מחלקות שיווק. זה CAC-אפס שאף סטארטאפ ממומן לא משיג. המוצר נמכר white-label / כ-line-item בריטיינר.

## הפרויקטים
1. **[Marketing Ops Hub](01-marketing-ops-hub.md)** 🟢 — ה-wedge הראשי. intake → approval → proofing. פער שוק אמיתי, אספקה מקומית דלילה.
2. **[HELIX Reports — דוחות שיווק](02-roi-reporting-engine.md)** 🎯 — **חינם לעסקים קטנים/סטארטאפים = כלי חדירה** ששובר את מודל התשלום של המתחרים ($44-79/חודש). נמכר רק לסוכנויות (white-label). מונטיזציה = רכישת לקוחות למותג HELIX, לא הכלי עצמו.
...
## תמחור (משותף)
מנוי **שטוח per-workspace** (נגד ה-nickel-and-diming ששוֹנא השוק) + wholesale לסוכנות + Request Desk חינם כפיתיון כניסה. WTP ישראלי: ~₪150-600/חודש/צוות.
