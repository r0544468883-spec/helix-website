...
## 🆕 מודול קמפיינים + A/B (2026-07) — audit UX
מסכים חדשים: `/[locale]/campaigns` (בונה) · `/[locale]/campaigns/[id]` (פירוט + פרסום). נבדק מול הצ'קליסט של [UX-MASTER-reference](UX-MASTER-reference.md):

| פריט | סטטוס | הערה |
|---|---|---|
| **RTL + עברית** | ✅ | `dir="rtl"`, כל הטקסט עברית, מספרים `he-IL` |
| **Loading state** | ✅ | `app/[locale]/loading.tsx` הגלובלי מכסה את הראוטים |
| **בנייה אינקרמנטלית (progress)** | ✅ | באנר "בונה 2/5…" + התוכן מופיע בהדרגה per-ערוץ |
| **Empty states** | ✅ | "עדיין אין קמפיינים" · "ממתין לבנייה…" · "אין פערים" |
| **Error states** | 🟡 | הודעות inline (`note`/`pubNote`); **לשדרג ל-toasts ממותגים** |
| **Confirm לפני פעולה חיה** | ✅ | אישור לפני "פרסם נבחרים" (פרסום אמיתי) — כרגע `window.confirm`, **לשדרג ל-ConfirmDialog ממותג** |
| **Feedback הצלחה** | 🟡 | טקסט inline ("✅ פורסמו N"); **לשדרג ל-toast + קונפטי** (כמו שאר המערכת) |
| **בחירה מרובה (checkboxes)** | ✅ | בחירת כמה גרסאות + מצב (אורגני/ממומן/סרטון) |
| **מטריקות per-גרסה** | ✅ | chips 👁/🖱/📊 + כפתור רענון |
| **נגישות (a11y)** | 🟡 | checkbox/labels יש; **חסר aria-label לכפתורי-אייקון (🎬/רענון)** |
| **מובייל** | 🟡 | grid מגיב; **לוודא touch targets 44px + אין scroll אופקי בטבלת RSA** |

### ⬜ מה נשאר להתאים ל-UX-MASTER
1. **Toasts ממותגים** במקום inline notes (הצלחה/שגיאה) — לאורך כל מודול הקמפיינים.
2. **ConfirmDialog ממותג** (glassmorphism) במקום `window.confirm`.
3. **קונפטי** על פרסום מוצלח (עקביות עם אישור/פרסום בשאר המערכת).
4. **aria-labels** לכפתורי-אייקון (🎬 צירוף סרטון · 🔄 רענון).
5. **מובייל** — בדיקת touch-targets + `overflow-x-auto` לטבלאות RSA/SEO.
6. **Onboarding** קצר למסך הקמפיינים (tooltip "מה זה 6×6").

> **מסקנה:** המסכים החדשים **פונקציונליים ומכסים את רוב הסטנדרטים** (RTL/loading/empty/confirm/progress), אבל שכבת ה-**delight/feedback הממותגת** (toasts/קונפטי/ConfirmDialog) עדיין לא הוחלה עליהם — משימת-המשך לפני live.
