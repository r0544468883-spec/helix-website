...
- **אוטומציית-ממומן מלאה ב-Meta 🆕:** `createMetaCampaign` בונה את כל ההיררכיה — **Campaign → Ad Set (תקציב יומי + targeting + optimization) → Ad per-גרסה (A/B)**. נוצר **PAUSED** (בטיחות — HELIX לא מוציא כסף לבד; המשתמש מפעיל ב-Ads Manager). כפתור "🚀 השק קמפיין ממומן" + שדות תקציב/objective. סוגר את הפער מול PowerAds.
