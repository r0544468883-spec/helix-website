...
| **📤 חיווט מנצח→הפצה + בחירת-מנצח אוטומטית 🆕** — `publishVariant` שולח את הווריאציה המנצחת לערוץ דרך מנוע ההפצה (`sendToChannel`) + לוג `publications`. `autoPickWinner` בוחר מנצח לפי ביצועים (conversions>clicks>impressions), ובהיעדר דאטה — לפי ציון-אנושיות | `app/actions-campaigns.ts`, `components/CampaignDetail.tsx` |
