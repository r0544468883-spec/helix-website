...
import { deliverCampaign, personalize } from '@/lib/email-deliver';
...
async function notifyOwnerByProduct(
  ...
  try {
    ...
    // profiles.email מוגבל ל-service_role מ-v18, אז הקריאה חייבת לעבור
    // בקליינט האדמין ולא בקליינט המשתמש.
...
export async function contactListing(listingId: string, fromName: string, message: string) {
  ...
  try {
    ...
    // listings.contact_email ו-profiles.email מוגבלים ל-service_role מ-v18.
    const db = createAdminClient() ?? supabase;
...
// שליחה ידנית מהמסך. אימות ההרשאות לסגמנט חי ב-deliverCampaign
// (lib/email-deliver.ts), כדי שגם הקרון המתוזמן יאכוף אותו.
export async function sendCampaign(campaignId: string, locale: string) {
  ...
  revalidatePath(`/${locale}/dashboard/email`);
  if (sent === -1) return { error: 'forbidden' };
