...
// Free-usage metering for /free-tools/differentiation. Counts runs per email in Supabase
// (reuses the content_tool_usage table with mode='differentiation'). Each run is two paid
...
export const MODE = 'differentiation';
...
export async function countUses(email: string): Promise<number> {
  ...
  if (!base || !key) {
    if (process.env.NODE_ENV === 'production') {
      console.error('differentiation-usage: SUPABASE_* unset, free quota cannot be enforced');
      return UNKNOWN_USES;
    }
  ...
  try {
    ...
    console.error('differentiation-usage: countUses failed', err);
    return UNKNOWN_USES;
  }
}
...
export async function recordUse(email: string): Promise<void> {
  ...
  try {
    ...
    console.error('differentiation-usage: recordUse failed', err);
  }
}
