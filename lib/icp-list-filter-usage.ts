...
// Free-usage metering for /free-tools/icp-list-filter. Counts runs per email in Supabase
// (reuses the content_tool_usage table with mode='icp-list-filter'). Each run is two paid
...
export const MODE = 'icp-list-filter';
...
export async function countUses(email: string): Promise<number> {
  ...
  if (!base || !key) {
    if (process.env.NODE_ENV === 'production') {
      console.error('icp-list-filter-usage: SUPABASE_* unset, free quota cannot be enforced');
      return UNKNOWN_USES;
    }
  ...
  try {
    ...
    console.error('icp-list-filter-usage: countUses failed', err);
    return UNKNOWN_USES;
  }
}
...
export async function recordUse(email: string): Promise<void> {
  ...
  try {
    ...
    console.error('icp-list-filter-usage: recordUse failed', err);
  }
}
