...
// Free-usage metering for /free-tools/gtm-intelligence. Counts runs per email in Supabase
// (reuses the content_tool_usage table with mode='gtm-intelligence'). Each run is two paid
// Claude calls (maker + critic), so the free limit is lower than the content tool's.
// Degrades to "unlimited" when Supabase isn't configured (localhost dev), so the tool still
// works without a DB. Same UNKNOWN_USES fail-closed discipline as content-usage.ts.

export const FREE_LIMIT = 2;
export const MODE = 'gtm-intelligence';
...
export async function countUses(email: string): Promise<number> {
  ...
  if (!base || !key) {
    if (process.env.NODE_ENV === 'production') {
      console.error('gtm-intelligence-usage: SUPABASE_* unset, free quota cannot be enforced');
      return UNKNOWN_USES;
    }
  ...
  try {
    ...
    console.error('gtm-intelligence-usage: countUses failed', err);
    return UNKNOWN_USES;
  }
}
...
export async function recordUse(email: string): Promise<void> {
  ...
  try {
    ...
    console.error('gtm-intelligence-usage: recordUse failed', err);
  }
}
