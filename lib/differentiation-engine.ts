...
const MAKER_SYSTEM = buildSystemPrompt({
  ...
  workflow: [
    ...
    'נסח משפט אסטרטגי אחד שמגדיר קטגוריה משלהם: מטרה, סקופ, יתרון (היתרון בא מה-VRINO, מהנכס שעבר). המשפט צריך לתאר את המרחב שבו הם יכולים להיות היחידים, לא הטובים ביותר. זה ה-category of one, המשך ישיר של הנכס שעבר VRINO והמרחב הפנוי מהאוקיינוס הכחול',
