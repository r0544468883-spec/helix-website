...
// Free-usage metering for /free-tools/cold-email-optimizer. Counts runs per email in Supabase
// (reuses the content_tool_usage table with mode='cold-email-optimizer'). Each run is two paid
// Claude calls (maker + critic). Degrades to "unlimited" when Supabase isn't configured
// (localhost dev), so the tool still works without a DB. Same UNKNOWN_USES fail-closed
// discipline as differentiation-usage.ts.

export const FREE_LIMIT = 3;
export const MODE = 'cold-email-optimizer';
...
export async function countUses(email: string): Promise<number> {
  ...
  if (!base || !key) {
    if (process.env.NODE_ENV === 'production') {
      console.error('cold-email-optimizer-usage: SUPABASE_* unset, free quota cannot be enforced');
      return UNKNOWN_USES;
    }
  ...
  try {
    ...
    console.error('cold-email-optimizer-usage: countUses failed', err);
    return UNKNOWN_USES;
  }
}
...
export async function recordUse(email: string): Promise<void> {
  ...
  try {
    ...
    console.error('cold-email-optimizer-usage: recordUse failed', err);
  }
}
