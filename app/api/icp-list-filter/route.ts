// ICP list-filter tool API, powers /free-tools/icp-list-filter. Sorts a pasted list of
// leads against an ICP definition into match / needs-review / no-match via a maker+critic
// Claude team. Same shape, gate and rate-limit convention as /api/differentiation.
...
export async function POST(req: Request) {
  ...
  if (body.mode === 'run') {
    ...
    helixAI.capture({ event: 'free_tool_used', distinctId: leadEmail, product: 'site', properties: { tool: 'icp-list-filter', mode: 'run' } });
    return NextResponse.json({
      ok: true,
      analysis: r.analysis,
