...
export default function FriendsOffer() {
  ...
  // reveal-on-scroll: animate each section as it enters, without pre-revealing
  // everything (that would kill the effect). Two safety nets keep content from
  // ever staying permanently hidden if the observer misfires.
  useEffect(() => {
    ...
    // safety 1: reveal only the first screen quickly so it is never blank
    const nearFold = window.setTimeout(() => {
      els.forEach((el) => {
        if (el.getBoundingClientRect().top < window.innerHeight) el.classList.add("in");
      });
    }, 1000);
    // safety 2: last resort, never leave anything hidden forever
  ...
  return (
    <main dir="rtl" className="fr">
      ...
      <style>{`
        ...
        /* reveal on scroll (content stays visible if JS is off) */
